...
if (isset($_REQUEST['edit'])) {
    $id = $_REQUEST['id'];
    $visitEdited = VisitDAO::show($id);
    $localSelected = $visitEdited->getLocal();
    $date = $visitEdited->getDate();
    $resume = $visitEdited->getResume();
    $action = "edit&id=" . $id;
}
...
<body>
    ...
    <div class="visits">
        ...
        <section class="bg-light p-3 rounded-3">
            <form method="POST" action="../controller/VisitController.php?<?php echo $action ?>" class="d-flex flex gap-2 gap-sm-3 flex-wrap">
                ...
                <div class="form-group col-xl-2 col-12">
                    <textarea class="form-control" name="resume" rows="1"><?php echo $resume ?></textarea>
                </div>
        ...
        <section class="bg-light p-3 rounded-2 mt-4">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php
                    $listVisits = VisitDAO::index();
                    foreach ($listVisits as $visit) {
                        if ($visit->getDone() == 1) {
                            echo '
                                <tr>
                                    <td class="visit done align-middle">' . $visit->getLocal() . '</td>
                                    <td class="visit done align-middle">' . $visit->getDate() . '</td>
                                    <td class="visit done align-middle">' . $visit->getResume() . '</td>
                                    <td class="d-flex flex-direction-row gap-2 h-100">
                                        <a href="../controller/VisitController.php?done&id=' . $visit->getId() . '" class="btn"><i class="bi bi-check2"></i></a>
                                        <a href="../controller/VisitController.php?delete&id=' . $visit->getId() . '" class="btn"><i class="bi bi-x"></i></a>
                                        <a href="?edit&id=' . $visit->getId() . '" class="btn"><i class="bi bi-pen"></i></a>
                                    </td>
                                </tr>
                                ';
                        } else {
                            echo '
                                <tr>
                                    <td class="visit align-middle">' . $visit->getLocal() . '</td>
                                    <td class="visit align-middle">' . $visit->getDate() . '</td>
                                    <td class="visit align-middle">' . $visit->getResume() . '</td>
                                    <td class="d-flex flex-direction-row gap-2 h-100">
                                        <a href="../controller/VisitController.php?done&id=' . $visit->getId() . '" class="btn"><i class="bi bi-check2"></i></a>
                                        <a href="../controller/VisitController.php?delete&id=' . $visit->getId() . '" class="btn"><i class="bi bi-x"></i></a>
                                        <a href="?edit&id=' . $visit->getId() . '" class="btn"><i class="bi bi-pen"></i></a>
